...
class PublishTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config = {
            'output': str(self.root/'out.m3u'),
            'report': str(self.root/'report.json'),
            'dropbox': {
                'path': '/folder/list.txt',
                'expected_file_id': 'id:original'
            }
        }
        self.data = r.render({
            'первый': r.Entry(
        ...
        self.report = {
            'status': 'ok',
            'playlist_sha256': hashlib.sha256(self.data).hexdigest(),
            'working_channels': 1,
    ...
    def save_report(self):
        Path(self.config['report']).write_text(
    ...
    def test_changed_bytes_are_rejected(self):
        Path(self.config['output']).write_bytes(
            self.data + b'\n'
        )
    ...
    def test_file_identity_and_revision_are_preserved(self):
        ...
        new = dict(
            old,
            size=len(self.data),
            content_hash=p.content_hash(self.data)
        )
        ...
        args = json.loads(
            upload.call_args.args[2]['Dropbox-API-Arg']
        )
